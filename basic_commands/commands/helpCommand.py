...
class HelpCommand:
    ...
    async def help(self, ctx: Context):
        ...
        embed.add_field(
            name='$$mute',
            value='Мьютит участника в определенном канале, сети или вовсе везде. Чтобы не передавать параметр пользователя можно просто ответить на его сообщение (необязательно оригинальное). В модальном окне можно передать время мьюта',
            inline=False
        )
        embed.add_field(
            name='$$unmute',
            value='Снимает с пользователя все мьюты',
            inline=False
        )
        embed.set_footer(
